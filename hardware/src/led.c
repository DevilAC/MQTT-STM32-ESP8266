...
void Led_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB | RCC_APB2Periph_GPIOF, ENABLE);	//打开GPIOA和GPIOC的时钟
	
	gpio_initstruct.GPIO_Mode = GPIO_Mode_Out_PP;									//设置为推挽输出模式
	gpio_initstruct.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_1 |GPIO_Pin_5;					//初始化Pin7、8、10
	gpio_initstruct.GPIO_Speed = GPIO_Speed_50MHz;									//承载的最大频率
	GPIO_Init(GPIOB, &gpio_initstruct);												//初始化GPIOC
	
	gpio_initstruct.GPIO_Pin = GPIO_Pin_7 | GPIO_Pin_8;											//初始化Pin12
	GPIO_Init(GPIOF, &gpio_initstruct);												//初始化GPIOA
...
void Led4_Set(LED_ENUM status)
{
	...
	GPIO_WriteBit(GPIOB, GPIO_Pin_5, status != LED_ON ? Bit_SET : Bit_RESET);	//status如果不等于LED_ON则返回Bit_SET，否则返回Bit_RESET。下同
...
void Led7_Set(LED_ENUM status)
{
	...
	GPIO_WriteBit(GPIOF, GPIO_Pin_7, status != LED_ON ? Bit_SET : Bit_RESET);
