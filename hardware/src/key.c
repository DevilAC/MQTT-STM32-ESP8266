...
void Key_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC | RCC_APB2Periph_GPIOA, ENABLE);	//打开GPIOC、GPIOD的时钟
	
	gpio_initstruct.GPIO_Mode = GPIO_Mode_IPU;									//设置为上拉输入模式
	gpio_initstruct.GPIO_Pin = GPIO_Pin_13;			//初始化Pin11、12、13
	gpio_initstruct.GPIO_Speed = GPIO_Speed_50MHz;								//承载的最大频率
	GPIO_Init(GPIOC, &gpio_initstruct);											//初始化GPIOC
	
	gpio_initstruct.GPIO_Pin = GPIO_Pin_0;
	GPIO_Init(GPIOA, &gpio_initstruct);											//初始化GPIOD
